...
bool debounceIgnition()
{
    ...
    // begin FSM where button presses register only on release, with debouncing feature
    ...
    case BUTTON_FALLING:
        ...
        // otherwise it will go back to BUTTON_UP. This is to ensure that the signal is in steady state and there is no bouncing.
        ...
        // increment debounce timer by 10 each loop
    ...
    case BUTTON_RISING:
        ...
        // otherwise it will go back to BUTTON_DOWN. This is to ensure that the signal is in steady state and there is no bouncing.
        if( accumulatedDebounceButtonTime >= TIME_DEBOUNCE_MS ) {
            if( ignition == OFF) {
                ignitionState = BUTTON_UP;
